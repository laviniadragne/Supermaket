...
; <left, right>   devine   <left ++ (reverse right), []>
;; A ++ B va fi (stream-cons (first A) <calcul-întârziat-append-între-restA-și-B>)
...
(define (enqueue_clasic x q)
  (match q
    [(queue left right size-l size-r)
     ; Am acces la campurile right, left si size-uri cu match si creez o noua
     ; structura cu ele, adaugand la inceputul lui right elementul dorit
     ; si marind size-ul
     (make-queue left (cons x right) size-l (add1 size-r))]))


(define (enqueue_helper_left q new_left)
  (match q
    [(queue left right size-l size-r)
     ; Am acces la campurile right, left si size-uri cu match si creez o noua
     ; structura cu ele, adaugand la inceputul lui right elementul dorit
     ; si marind size-ul
     (make-queue new_left '() (+ size-l size-r) 0)]))


(define (enqueue x q)
  ; daca lungimea stream-ului left e cel putin cat lungimea lui right, in urma enque-ului
  (if (<= (queue-size-l q) (queue-size-r q))
      (enqueue_helper_left (enqueue_clasic x q) (rotate (queue-left q) (cons x (queue-right q)) '()))
      (enqueue_clasic x q)))
...
(define (dequeue_clasic q)
   (match q
    [(queue left right size-l size-r)
     ; Am acces la campurile right, left si size-uri cu match si creez o noua
     ; structura cu ele, adaugand la inceputul lui right elementul dorit
     ; si marind size-ul
     (make-queue (stream-rest left) right (sub1 size-l) size-r)]))


(define (dequeue_helper q new_left)
  (match q
    [(queue left right size-l size-r)
     ; Am acces la campurile right, left si size-uri cu match si creez o noua
     ; structura cu ele, adaugand la inceputul lui right elementul dorit
...
(define (dequeue q)
  (if (<= (queue-size-l q) (queue-size-r q))
      (dequeue_helper (dequeue_clasic q) (rotate (stream-rest (queue-left q)) (queue-right q) '()))
      (dequeue_clasic q)))
